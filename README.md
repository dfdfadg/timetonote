...
**Articles never include their category in the URL.** `/home-problems/why-is-my-house-so-dusty` permanently redirects to `/why-is-my-house-so-dusty`. Canonical URLs use `https://timetonote.com` (no `www`, no trailing slash); `www` and trailing-slash URLs 308-redirect to the canonical form.
